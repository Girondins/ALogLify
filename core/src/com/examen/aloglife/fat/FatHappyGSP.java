...
public class FatHappyGSP extends ApplicationAdapter {
    ...
    public FatHappyGSP(float centerX, float centerY){
    ...
    public void create () {
        ...
        atlas = new TextureAtlas(Gdx.files.internal("fat/fatHappyGSP/FatGSP_tex.atlas"));
        SkeletonJson json = new SkeletonJson(atlas); // This loads skeleton JSON data, which is stateless.
        json.setScale(1.2f); // Load the skeleton at 50% the size it was in Spine.
        SkeletonData skeletonData = json.readSkeletonData(Gdx.files.internal("fat/fatHappyGSP/FatGSP.json"));
    ...
    //    stateData.setMix("lookatshoes", "Idle", 0.2f);
        ...
        timer.schedule(new FatHappyGSP.Animater(),2000);
            ...
            while (true) {
                switch (task) {
                    ...
                    case 1:
                        ...
                        if(isFlip == false){
                            for (float i = centerX; i > centerX-145 ; i = i - 4) {
                                skeleton.setX(i);
                                try {
                                    Thread.sleep(100);
